...
После первой настройки YouTube-плагина:
1. Открой Kodi → Add-ons → My add-ons → Video add-ons → YouTube → Configure.
2. Settings → MrMC/Cast → **Enable remote control via UPnP & DIAL** = ON.
3. Перезапусти Kodi (или ребутни N100).

Теперь в YouTube-приложении на телефоне (в той же Wi-Fi сети!) появляется иконка каста.
Жмёшь — выбираешь «Kodi». Дальше телефон отдаёт Kodi только видео-ID, а само видео Kodi
тянет напрямую через свой плагин.
