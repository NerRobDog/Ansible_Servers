...
- Ключевые параметры:
  ...
  - `tailscale_hostname` — имя узла в tailnet. По умолчанию алиас хоста из
    inventory, `_` заменяется на `-` (`wrt_nab` → `wrt-nab`). Пустая строка
    оставляет имя, которое выставил провайдер.
- Поведение:
  - если хост уже авторизован, `tailscale up` не выполняется;
  - если хост не авторизован, нужен ключ из GitHub Secrets;
  - если имя узла в tailnet не совпадает с `tailscale_hostname`, роль вызывает
    `tailscale set --hostname`. Провайдеры ставят в hostname ОС внутренние ID
    (`6019769891-arcoknq`, `bumpy-red`), и без этого узел регистрируется под ними.
    Смена имени меняет MagicDNS-адрес `<имя>.<tailnet>.ts.net`; IP не меняется.
    Роль сверяет имя, которое сообщает клиент (`Self.HostName`). Если машину
    переименовали вручную в админке Tailscale, админка важнее, и роль этого не видит.
