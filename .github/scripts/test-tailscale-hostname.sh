...
out="$(run_play tw_spb_1)" || { echo "$out" >&2; fail "drift: play failed"; }
[[ "$(calls)" == "set --hostname=tw-spb-1" ]] || fail "drift: expected one rename to tw-spb-1, got: $(calls)"
grep -Eq 'changed=1 ' <<< "$out" || fail "drift: play did not report changed=1"
echo "PASS: drifted hostname is renamed to the alias"

# Converged: second run changes nothing.
out="$(run_play tw_spb_1)" || { echo "$out" >&2; fail "idempotence: play failed"; }
[[ "$(calls)" == "set --hostname=tw-spb-1" ]] || fail "idempotence: unexpected extra call: $(calls)"
grep -Eq 'changed=0 ' <<< "$out" || fail "idempotence: play reported a change"
echo "PASS: matching hostname is left alone"

# Check mode reports the rename but does not perform it.
printf 'bumpy-red-1' > "$FAKE_TS_STATE"; : > "$FAKE_TS_CALLS"
run_play ae-us-1 --check > "$work/check.out" 2>&1 || { cat "$work/check.out" >&2; fail "check mode: play failed"; }
[[ -z "$(calls)" ]] || fail "check mode: tailscale set was called: $(calls)"
[[ "$(cat "$FAKE_TS_STATE")" == "bumpy-red-1" ]] || fail "check mode: hostname changed"
echo "PASS: check mode does not rename"

# An explicit empty hostname opts the host out.
run_play ae-us-1 -e tailscale_hostname= > "$work/optout.out" 2>&1 || { cat "$work/optout.out" >&2; fail "opt-out: play failed"; }
[[ -z "$(calls)" ]] || fail "opt-out: tailscale set was called: $(calls)"
echo "PASS: empty tailscale_hostname opts out"
...
if run_play ae-us-1 -e 'tailscale_hostname=Bad_Name' > "$work/invalid.out" 2>&1; then
  fail "invalid: play succeeded"
fi
grep -q "not a valid" "$work/invalid.out" || fail "invalid: missing validation message"
[[ -z "$(calls)" ]] || fail "invalid: tailscale set was called: $(calls)"
echo "PASS: invalid hostname is rejected"
